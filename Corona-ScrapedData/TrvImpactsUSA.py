...
url = "https://news.google.com/topics/CAAqBwgKMKO5lwswlOKuAw?hl=en-US&gl=US&ceid=US%3Aen"
...
for wrd in TrvHeadlines:
    ...
    TrvTitles.append(re.sub(r'<a class="DY5T1d" .+?>', '', str(f_part), flags=re.MULTILINE)+
    re.sub(r'</span>', '', str(d_part), flags=re.MULTILINE))

pd.DataFrame(TrvTitles).to_csv('TrvUsaNews_'+str(date.today())+'.csv')
